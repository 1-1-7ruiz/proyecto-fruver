...
document.addEventListener('DOMContentLoaded', function () {
    ...
    fetch('http://127.0.0.1:8000/productos/productos/leguminosas')
        .then(response => {
    ...
    data.forEach(leguminosa => {
        const card = document.createElement('div');
        card.className = 'card-wrapper';
        card.dataset.id = leguminosa.id; // 👈 guardamos el id en el dataset
        card.innerHTML = `
            <ul class="card-list">
                <li class="card-item">
                    <div class="product-img"><img src="${backendUrl + leguminosa.foto}" alt="${leguminosa.nombre}" class="card-image"></div>
                    <h2 class="user-name">${leguminosa.nombre}</h2>
                    <div class="precio">
                        <p>$<span class="precio-producto">${leguminosa.precio}</span></p>
                    </div>
                    <div class="contador-wrapper">
