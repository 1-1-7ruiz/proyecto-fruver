...
document.addEventListener('DOMContentLoaded', function () {
    ...
    fetch('http://127.0.0.1:8000/productos/productos/verduras')
        .then(response => {
    ...
    data.forEach(verdura => {
        ...
        card.dataset.id = verdura.id; // 👈 guardamos el id en el dataset
        card.innerHTML = `
            <ul class="card-list">
                <li class="card-item">
                    <div class="product-img"><img src="${backendUrl + verdura.foto}" alt="${verdura.nombre}" class="card-image"></div>
                    <h2 class="user-name">${verdura.nombre}</h2>
                    <div class="precio">
                        <p>$<span class="precio-producto">${verdura.precio}</span></p>
                    </div>
                    <div class="contador-wrapper">
